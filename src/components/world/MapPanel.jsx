  ...
  return (
    <div className="space-y-4">
      <CardHeader className="!mb-0">
        <CardTitle className="flex items-center gap-2"><Map size={16} /> 地图图谱</CardTitle>
        <div className="flex gap-2">
          <button onClick={openConnCreate} className="flex items-center gap-1 text-sm px-3 py-1.5 rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-200 transition"><ArrowRight size={14} /> 连接</button>
          <button onClick={openRegionCreate} className="flex items-center gap-1 text-sm px-3 py-1.5 rounded-lg bg-sky-600 hover:bg-sky-500 text-white transition"><Plus size={14} /> 区域</button>
        </div>
      </CardHeader>
      ...
      <div className="bg-slate-900/40 border border-slate-700/40 rounded-lg p-3">
        <h4 className="text-xs font-medium text-sky-400 mb-2">区域分布</h4>
        {regions.length > 0 ? renderRegionTree(rootRegions) : <div className="text-xs text-slate-500 py-2">暂无区域</div>}
      </div>

      {/* 连接表 */}
      {connections.length > 0 && (
        <div className="bg-slate-900/40 border border-slate-700/40 rounded-lg p-3">
          <h4 className="text-xs font-medium text-sky-400 mb-2">区域连接</h4>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-slate-700/40 text-xs text-slate-500">
                <th className="text-left py-2 px-2">From</th><th className="text-left py-2 px-2">类型</th><th className="text-left py-2 px-2">To</th><th className="text-left py-2 px-2">耗时</th><th className="text-right py-2 px-2">操作</th>
              </tr></thead>
              <tbody>
                {connections.map(c => (
      ...
      {!loading && regions.length === 0 && (
        <div className="text-center text-slate-500 text-sm py-8 bg-slate-900/20 rounded-lg border border-dashed border-slate-700">暂无地图区域，点击上方按钮添加</div>
      )}
      ...
      <Modal open={regionModal} onClose={() => setRegionModal(false)} title={editingRegion ? '编辑区域' : '新增区域'} onConfirm={saveRegion} confirmDisabled={!regionForm.name}>
        <Input value={regionForm.name} onChange={e => setRegionForm({ ...regionForm, name: e.target.value })} placeholder="区域名称" />
        <div className="grid grid-cols-2 gap-3">
          ...
          <select value={regionForm.parentId} onChange={e => setRegionForm({ ...regionForm, parentId: e.target.value })} className="w-full px-3 py-2 rounded-lg border border-slate-600 bg-slate-900/60 text-slate-50 text-sm">
            <option value="">无上级区域</option>
            {regions.filter(r => r.id !== editingRegion?.id).map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
          </select>
        </div>
        <Textarea value={regionForm.description} onChange={e => setRegionForm({ ...regionForm, description: e.target.value })} placeholder="区域描述..." />
        <Input value={regionForm.tags} onChange={e => setRegionForm({ ...regionForm, tags: e.target.value })} placeholder="标签（逗号分隔）" />
      </Modal>

      <Modal open={connModal} onClose={() => setConnModal(false)} title="新增区域连接" onConfirm={saveConn} confirmDisabled={!connForm.fromRegionId || !connForm.toRegionId}>
        <div className="grid grid-cols-2 gap-3">
          <div>
        ...
        <Input value={connForm.travelTime} onChange={e => setConnForm({ ...connForm, travelTime: e.target.value })} placeholder="通行时间（如：3天）" />
        <Textarea value={connForm.description} onChange={e => setConnForm({ ...connForm, description: e.target.value })} placeholder="连接描述..." />
      </Modal>
    </div>
  );
}
