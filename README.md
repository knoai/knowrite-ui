...
**Knowrite UI** 是 [`knowrite`](https://github.com/igoryu/knowrite) 小说创作引擎的配套前端，通过标准 HTTP / SSE 连接后端，零配置联调，开箱即用。
...
- Node.js 24+
- 后端服务 [`knowrite`](https://github.com/igoryu/knowrite) 已启动（默认 `http://localhost:8000`）

### 安装

```bash
# 克隆前端仓库
git clone https://github.com/igoryu/knowrite-ui.git
cd knowrite-ui
...
- **模型配置**：选择默认 Provider 和模型、配置多模型轮换
...
后端 [`knowrite`](https://github.com/igoryu/knowrite) 采用 NC-1.0 协议（非商业个人使用），前端与后端协议分离，前端代码不受后端 NC-1.0 限制。

---

> 后端仓库：[`knowrite`](https://github.com/igoryu/knowrite)（NC-1.0）| 前端仓库：`knowrite-ui`（MIT）| 路线图：`docs/ROADMAP.md`
